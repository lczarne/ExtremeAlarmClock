...
- (void)prepareScrollView:(UIScrollView*)scrollView withCounter:(int)numbers {
    ...
    scrollView.contentSize = CGSizeMake(scrollView.bounds.size.width/3, scrollView.bounds.size.height*numbers);
    ...
    CGRect frame = (CGRect){CGPointMake(0, -scrollView.bounds.size.height), CGSizeMake(scrollView.bounds.size.width, scrollView.bounds.size.height*numbers)};
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    {
        _number1Value = page;
        counterNumbersValue = 13;
    }    
    if (scrollView == self.number2)
    {
        _number2Value = page;
        counterNumbersValue = 60;
    }
...
- (void)awakeFromNib {
    ...
    if (_isStandIn)
        return;
    counterNumbersValue = 13;
    [self prepareScrollView:self.number1 withCounter:counterNumbersValue];
    counterNumbersValue = 60;
    [self prepareScrollView:self.number2 withCounter:counterNumbersValue];
    [self prepareScrollView:self.number3 withCounter:counterNumbersValue];
...
//    }
    ...
    return [NSString stringWithFormat:@"%d : %d",_number1Value, _number2Value];

    //return [NSString stringWithFormat:@"%d,:,%d",self.number1, self.number1
}
